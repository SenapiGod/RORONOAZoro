...
async def lego(event):
    ...
    try:
        ...
        await telethn.send_file(
            event.chat_id, file=fname, caption=f"ʟᴏɢᴏ ɢᴇɴᴇʀᴀᴛᴇᴅ ʙʏ ʀᴏʀᴏɴᴏᴀ ᴢᴏʀᴏ \nRequested By:- [{event.sender.first_name}](tg://user?id={event.sender.id})\nCreated By: [ ʀᴏʀᴏɴᴏᴀ ᴢᴏʀᴏ ](https://t.me/Demongod_Zorobot)"
        )
        await pesan.delete()
        if os.path.exists(fname):
            os.remove(fname)
    except Exception as e:
        await event.reply(f"ғʟᴏᴏᴅᴡᴀɪᴛ ᴇʀʀᴏʀ, ʀᴇᴩᴏʀᴛ ᴛʜɪs ᴀᴛ @ItAchixoBot_Suppor") 
...
__help__ = """
Zoro can create some beautiful and attractive logo for your profile pics.
